...
# ==================== HELPERS ====================
def get_cloudflared_hint():
    """Detect OS/Distro and return specific install command."""
    sys_name = platform.system()
    if sys_name == "Darwin":
        return "brew install cloudflared"
    if sys_name == "Windows":
        return "winget install Cloudflare.cloudflared  OR  choco install cloudflared"
    
    if sys_name == "Linux":
        # Minimal root check
        is_root = False
        try: is_root = os.getuid() == 0
        except: pass
        sudo = "" if is_root else "sudo "

        # Detect via os-release
        distro_id = ""
        ...
        if distro_id in ["ubuntu", "debian"]:
            return f"{sudo}apt install cloudflared"
        if distro_id in ["fedora", "rhel", "centos"]:
            return f"{sudo}dnf install cloudflared"
        if distro_id == "arch":
            return f"{sudo}pacman -S cloudflared"
            
        # Fallback to package manager check if os-release is weird
        if shutil.which("apt"): return f"{sudo}apt install cloudflared"
        if shutil.which("dnf"): return f"{sudo}dnf install cloudflared"
        if shutil.which("pacman"): return f"{sudo}pacman -S cloudflared"
...
def cleanup(sig=None, frame=None):
    ...
    # First, try to terminate gracefully (allows curses to cleanup)
    for p in _procs:
        try:
            p.terminate()
        except Exception:
    ...
    # Give the agent a moment to restore terminal state
    time.sleep(0.5)
...
def check_cloudflared():
    ...
    hint = get_cloudflared_hint()
    log("     ", f"Install: {hint}", Y)
    log("     ", "Or download: https://github.com/cloudflare/cloudflared/releases", Y)
    return None

def check_curses():
    """On Windows, check for windows-curses."""
    if platform.system() == "Windows":
        try:
            import _curses
            log("OK   ", "curses (via windows-curses) found", G)
        except ImportError:
            log("WARN ", "windows-curses not found!", Y)
            log("     ", "Run: pip install windows-curses", Y)
            return False
    return True
